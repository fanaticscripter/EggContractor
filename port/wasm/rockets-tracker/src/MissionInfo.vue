<template>
  <ul
    v-if="activeMissions && activeMissions.length > 0"
    class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 mx-4 xl:mx-0 my-4"
    :class="[activeMissions.length >= 4 ? 'lg:grid-cols-4' : 'lg:grid-cols-3']"
  >
    <li
    ...
    >
      <div class="flex-1 flex flex-col p-6">
        <div
          ...
          :class="[durationTypeFgClass(mission.durationTypeDisplay)]"
        >
          <img
        ...
        <div class="mt-1 flex-grow flex flex-col">
          <div>
            <span
              ...
              :class="[durationTypeBgClass(mission.durationTypeDisplay)]"
              >{{ mission.durationTypeDisplay }}</span
            >
          </div>
          ...
          <div
            v-if="mission.returnTimestamp > 0"
          ...
          <div
            v-else-if="
              mission.statusDisplay === 'Fueling' && mission.fuels && mission.fuels.length > 0
            "
            class="mt-1"
          >
            <img
              ...
              v-tippy="{
                content:
                  fuel.amountDisplay +
                  ' (this value from your save data may lag behind your actual progress)',
              }"
            />
          </div>
        </div>
  ...
  <div v-else class="text-center mt-6">No active mission. You should start one!</div>
    ...
    >
      ...
      >
        ...
        <span
          ...
          :class="[notificationsOn ? 'bg-green-400' : 'bg-gray-200']"
        ></span>
        <span
          aria-hidden="true"
  ...
  <div class="mt-6">
    <h2 class="mx-4 my-4 text-center text-md leading-6 font-medium text-gray-900">
      Mission statistics
    </h2>
    ...
    <div v-if="unlockProgress" class="-mt-2 mb-2 -space-y-1">
      <div
        v-if="
          unlockProgress.nextShipToLaunch &&
          (!unlockProgress.nextShipToUnlock ||
            unlockProgress.nextShipToLaunch.name !== unlockProgress.nextShipToUnlock.name)
        "
        class="text-sm text-center space-x-1"
      >
        <img class="inline w-8 h-8" :src="iconURL(unlockProgress.nextShipToLaunch.iconPath, 128)" />
        <span class="whitespace-nowrap"> {{ unlockProgress.nextShipToLaunch.name }} unlocked </span>
      </div>
      ...
      <div v-if="unlockProgress.nextShipToUnlock" class="text-sm text-center space-x-1">
        ...
        <span class="whitespace-nowrap">
          ...
          <span class="font-medium">
            {{ unlockProgress.nextShipToUnlock.launchesDone }} /
            {{ unlockProgress.nextShipToUnlock.launchesRequired }}
          </span>
          <span
            class="inline-flex items-center space-x-1 ml-1"
            v-tippy="{
              content:
                'This is the accumulative mission time for all remaining required launches, including mission time for previous locked ships (if any), divided by max concurrency (3 for Pro Permit holders, 1 otherwise). Remaining timer on your active mission(s) is not included.',
            }"
          >
            ({{ unlockProgress.nextShipToUnlock.accumulativeMissionTimeRequired }}
            <svg
              class="h-4 w-4 text-gray-400"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
      ...
      <template v-if="unlockProgress.furtherShipsToUnlock">
        ...
        <template v-if="furtherShipsToUnlockExpanded">
          <div
            v-for="ship in unlockProgress.furtherShipsToUnlock"
            :key="ship.name"
            class="text-sm text-center space-x-1"
          >
    ...
    <div class="mx-4 my-4 xl:mx-0 text-xs">
      ...
      <ul class="list-disc">
        <li>
          Launched active missions are counted in mission statistics; fueling active missions are
          not.
        </li>
        <li>
          There might be a delay between your local mission state updates and server state pulled by
          this site.
        </li>
      </ul>
    </div>
  ...
  <div class="mx-4 xl:mx-0 my-4">
    ...
    <div class="w-full max-w-xs mx-auto my-2">
      ...
      <select
        id="launch-log-filter"
        name="launch-log-filter"
...
export default {
  ...
  computed: {
    filteredLaunchLog() {
      ...
      return Object.keys(date2missions)
        .sort()
        .reverse()
        .map(date => ({
          date,
