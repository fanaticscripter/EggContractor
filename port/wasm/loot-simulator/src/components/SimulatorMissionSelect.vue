<template>
  <!-- TODO: This components almost duplicates 100% of SimulatorItemSelect. Need
  to find a way to make a type-safe generic component (something to the effect
  of defineComponent with generic types in a SFC). -->
    ...
    >
      ...
      >
        <li
          ...
          :class="mission.id === active?.id ? 'text-white bg-blue-600' : 'text-gray-900'"
          @mousedown="selectMission(mission)"
          @mouseenter="activateMission(mission)"
          @mouseleave="deactivateMission()"
        >
          <div class="flex items-center">
            <img :src="iconURL(mission.iconPath, 64)" class="flex-shrink-0 h-6 w-6" />
            <span
              class="ml-2 block truncate flex-shrink-0"
              :class="mission.id === selected?.id ? 'font-semibold' : 'font-normal'"
            >
              {{ mission.display }}
            </span>
            <span
              class="ml-2 truncate"
              :class="mission.id === active?.id ? 'text-blue-200' : 'text-gray-500'"
            >
              ({{ mission.count }} on file)
            </span>
          </div>

          <span
            v-if="mission.id === selected?.id"
            class="absolute inset-y-0 right-0 flex items-center pr-4"
            :class="mission.id === active?.id ? 'text-white' : 'text-blue-600'"
          >
...
export default defineComponent({
  ...
  props: {
    modelValue: {
      type: Object as PropType<MissionSelectSpec>,
      required: true,
    },
  },
  emits: {
  ...
  setup(props, { emit }) {
    ...
    const dropdownListEntryIndex = (id: MissionId): number => {
